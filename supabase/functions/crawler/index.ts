...
Deno.serve(async (req) => {
  ...
  if (action === "status") {
    ...
    return Response.json({ prospects: counts, cases, filings });
  }
  ...
  const fs = await fetchAndScore(8);
  return Response.json({ discovered: found, ...fs, next: "extract picks up promoted prospects on its own cron" });
});
